...
const scoreUpperList = defineModel('scoreUpperList', {type: Object, default: {}});
const scoreLowerList = defineModel('scoreLowerList', {type: Object, default: {}});
...
const countUpper = () => {
    ...
    for (const entry of scoress(scoreUpperList)) {
...
const sumUpper = () => {
    const scoresU = scoress(scoreUpperList);
...
const multiYahtzee = () => {
    moreYahtzee = arrayEntry(scoress(scoreLowerList), 'id', 'yahtzee').final === 50 && yahtzeeNumber.value != 0;
};
...
const klikable = () => {
    const scoresU = scoress(scoreUpperList);
    const scoresL = scoress(scoreLowerList);
...
const countLower = () => {
    ...
    for (const entry of scoress(scoreLowerList)) {
        if (entry.locked != back) {
...
<template>
    <div>{{ scoreUpperList }}</div>
    <div>{{ scoreLowerList }}</div>
</template>
